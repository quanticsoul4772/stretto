...
A generative music synthesizer. Plays live on Linux (PulseAudio/PipeWire) or Windows (waveOut), or renders to a 48 kHz stereo WAV file. C99 and Zig, no malloc — all runtime state lives in one static 128 KB arena. Output is deterministic: the same `--seed` and flags always produce byte-identical audio.
...
make test-unit       # 241 unit tests across all pure-synth modules + keys + MIDI + resume
...
| `tests/unit/test_*.c` | 241 unit tests across arena, effects, voice, gen, lsystem, chord_progression, section, density, motif, mixer, wav, keys, midi, resume |
