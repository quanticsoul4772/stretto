...
## Recent: the synth engine is Zig (087-107)

All eight in-scope modules are ported: density, chord_progression, motif, section, lsystem, effects, voice and gen. Each `.h` is unchanged, every unit test passes without modification, and the golden hash is byte-identical throughout. gcc still compiles the remaining C and links every target, so GNU Make stays the build system - this is per-module objects, not a toolchain migration. arena, mixer, wav, ui, keys, main, the audio backends and the five table generators stay C by design.
...
- **Size**: 52 504 B stripped / 31 624 B packed / 45 056 B Windows packed (CI run 30781463812), leaving 9.2% / 10.1% / 9.1%. The binding constraint is no longer any cap but the **page cliff**: 484 B of headroom against a 256 B advisory, down from 3 428 B. The next feature that crosses a 4 KB boundary costs 4 096 B stripped whatever the caps report.
