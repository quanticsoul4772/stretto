...
| Binary | Size |
|---|---|
| Linux `synth` (stripped, links libpulse + libasound) | ~49 KB (52 600 B per CI run 30784977189, measured 2026-08-03 with nine modules in Zig; was 49 128 B all-C per run 29211125164; STRIP_TARGET = 57 344 B as of the v1.4.0 realignment, leaving 4 744 B / 8.3 %; was 51 200 B) |
| Linux `synth.packed` (UPX) | ~30 KB (31 728 B per CI run 30784977189, measured 2026-08-03 with nine modules in Zig; was 30 048 B all-C per run 29211125164); PACK_TARGET = 34 816 B per the Makefile enforces the Constitution v1.4.0 cap, leaving 3 088 B / 9.7 %; it was 30 720 B. **Still the binding budget of the three.** v1.4.1 settled the caps at these values — see the amendment for why re-tightening would have gone past the project's own precedent. The page cliff (**324 B**, against a 256 B advisory) now constrains growth more than any cap |
...
ui.c      / .h          cross-platform terminal raw mode, oscilloscope
                        grid renderer, status panel/row builders, help
...
**Cross-platform bit-exactness scope (Constitution Principle III v1.0.1).** With `--seed N`, the same render on any of the supported build targets (Linux glibc + Windows winmm, both little-endian x86) produces byte-identical output by code construction: the runtime engine is integer-only across voice / gen / mixer / effects (no `double` / `float` in any hot path), the PRNG is `xorshift32` over `uint32`, the build-time `gen_*_table.c` programs round doubles to committed-header bytes via the deterministic IEEE-754 `(int)(x + 0.5)` contract, and `wav.c` emits native-endian RIFF (`fwrite(&uint16/uint32, ...)`) which is little-endian on both supported targets. The `'--render'` path bypasses `audio_pulse.c` / `audio_winmm.c` entirely — those files affect live playback only, not the bit-exact regression. The bit-exact regression is gated on the Linux CI runner only; CI cannot execute the Windows binary. The cross-platform half of the invariant is covered by `tests/test_crossplatform.sh` (`make test-crossplatform`, dev-box only — needs WSL interop or Windows to run the .exe): it renders the four multiseed seeds plus the 16 s golden seed through BOTH binaries and byte-compares (first verified 2026-07-11: all five pairs identical, both sides matching the committed golden). Scope note: both binaries share the build-host-generated tables, so the script proves the runtime engine's platform independence; table generation itself is pinned by building releases on one runner image. See `.specify/memory/constitution.md` Principle III v1.0.1 and the spec-kit pipeline section above for the precise invariant claim.
...
**Producer (ALSA, Linux)** — `audio_midi_linux.c` opens a `SND_SEQ_OPEN_INPUT` handle with the client name `Stretto` and one generic application port `Stretto Input` (end-users subscribe their controller with `aconnect <controller> 129:0`). A single `pthread_create` worker blocks on `snd_seq_event_input` via a 100 ms `poll` / `snd_seq_poll_descriptors` cycle and writes through `audio_midi_enqueue`. Per event: `NOTEON` / `NOTEOFF` / `CONTROLLER` / `PITCHBEND` parse into a `midi_event_t` (ALSA channel 0..15 → MIDI 1..16, key / value from structured `ev->data.{note, control}` fields; bend arrives pre-combined and centered −8192..8191 in `control.value` and is re-split into the LSB/MSB wire packing — 072/FR-015); `SND_SEQ_EVENT_PORT_EXIT` / `SND_SEQ_EVENT_CLIENT_EXIT` flip a cooperative-shutdown atomic (`g_run`) so the worker returns within one poll window (`pthread_join` then completes the close — FR-034 graceful disconnect, **no auto-reconnect**). Sysex / clock / program-change / active-sensing fall through silently (queue slots are precious; 256 entries per FR-040). Enumeration (`audio_midi_linux_list_devices`) walks `snd_seq_query_next_client` + `snd_seq_query_next_port` with the same dual filter as the wildcard open path (`SND_SEQ_PORT_CAP_READ` + `SND_SEQ_PORT_TYPE_MIDI_GENERIC` — the once-documented `MIDI_KEYBOARD` constant does not exist in upstream alsa-lib), so the list shows exactly what `--midi` would subscribe to; each entry's index is the `(client<<8)|port` encoding that `audio_midi_linux_init` decodes.

**Producer (winmm, Windows)** — Win32 manages the service thread. `audio_midi_winmm.c` opens the device via `midiInOpen(... CALLBACK_FUNCTION)`, prepares + adds a 4-buffer pool of 4096 B sysex headers (`MIDIHDR` re-prepared + re-added on each `MIM_LONGDATA` so the pool keeps delivering), then arms the input flow with `midiInStart`. Each `MIM_DATA` callback parses the short message (`status & 0xF0` selects Note On / Off / CC / Pitch Bend; channel `+1` from 0..15 to MIDI 1..16; data1 = key / CC# / bend LSB; data2 = velocity / value / bend MSB — 072/FR-015) into the same `midi_event_t` shape and writes via the same `audio_midi_enqueue`. `MIM_CLOSE` triggers the symmetric FR-034 path. Sysex payload contents are discarded in v1 (SysEx is out of scope). `MIM_ERROR` / `MIM_LONGERROR` are surfaced as drops counted by `audio_midi_drop_count`. Enumeration (`audio_midi_winmm_list_devices`) walks `midiInGetNumDevs` + `midiInGetDevCaps`, populating `midi_input_device_t { .index, .name = truncated_szPname }`.
...
**Status panel (074).** Terminals with ≥ 20 rows get a five-line full-word panel (`ui_build_status_panel`): version/seed/bar/section/ms-per-bar/swing, scale/chord/tension/motif, filter words + cutoff/resonance/lfo, effects, and voices grouped by role band. Shorter terminals fall back to the compact single row (`ui_build_status_row`, now including `Sw:`). Rich layout uses 5 lines + a (th−6)-row scope, fallback 1 line + (th−2) — both th−1 total. Every status line passes through `clamp_line`, a single SGR-aware post-pass that counts only *visible* columns (each fused color literal carries ~5 invisible bytes) and cuts at the last space boundary at tw−1 — this also fixed the pre-074 row wrap below ~72 columns that scrolled the scope every frame. Both builders are exported for PTY-free unit tests (the `ui_strip_sgr` precedent). The help overlay (`?`) is built on demand into a 4 KB BSS buffer: one row per `PARAM_FLAGS` entry (live key, flag name, current value, range — names and ranges shared with the CLI parser via the table, which lives in ui.c so unit-test binaries that link ui.o without main.o resolve it), colorless by design so `ui_strip_sgr` remains the single NO_COLOR strip site.
...
| `make test-unit` | 240 unit tests across the `tests/unit/test_*.c` files (arena, effects, voice, gen, lsystem, midi, chord_progression, section, density, motif, mixer, wav, keys, resume, main, ui) using the hand-rolled framework in `tests/unit/test.h`. |
...
Total: 240 unit tests across 16 modules (the MIDI suite covers: US1 scale-degree + octave clamp + velocity + voice-stealing + no-midi byte-identity, US2 CC dispatcher + multi-CC additive composition + reserved-CC no-op slots + sustain pedal + All Notes Off, US3 channel filter + enumeration + wildcard sentinel, plus the malformed-channel guard and the deterministic 50k-event fuzz).
...
ci.yml defines two jobs. The `sanitizers` job (066) is a single `make test-asan` step — ASan + UBSan (fatal) over the unit suite + a render on a fresh checkout, `timeout-minutes: 15`. Since 075 it is a **required check** in the main-protection ruleset alongside `build-test-coverage` — both are matched **by rendered job name**, so renaming either job (or adding a `name:` key) orphans the required context and blocks every merge with "Expected — waiting for status"; update the ruleset in the same change. The main `build-test-coverage` job's 19 explicit steps render as UI rows 2–20 (row 1 is the auto-prepended `Set up job`):
...
| 9 | Unit tests | `make test-unit` (196 tests across 14 modules) |
