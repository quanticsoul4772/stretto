...
- **2026-07-08** — Constitution v1.0.x → v1.1.0: Principle I Linux stripped binary budget bumped ≤24 KB → ≤50 KB per PR #116 (019-realign-strip-target). The Makefile `STRIP_TARGET` bumped 24576 → 51200 (~50 KB). T061's underlying threshold is now `STRIP_TARGET = 51200` per Makefile (the live target); the parenthetical "(24 KB stripped target)" in the T061 task line above is preserved AS-IS per the integrity-preserving pattern. PR #119 added the `Binary size budget gate` ci.yml step, which hard-fails the workflow (exit 1) when `linux_synth_stripped` exceeds STRIP_TARGET, so v1.1.0's cap is now machine-enforced at PR-merge time rather than only Makefile-asserted at `make size` invocation. The current cap is ≤50 KB per `.specify/memory/constitution.md` Principle I amendment paragraph. (PR #117's v1.1.0 spec-doc propagation did not refresh on disk for this 001 baseline tasks; PR #124 / this PR closes the catchup by appending this entry without editing the T061 task line's "(24 KB stripped target)" parenthetical.)

- **2026-07-08** — Constitution v1.1.0 → v1.2.0: Principle I Linux UPX-packed binary budget added ≤30 KB per PR #121. The Makefile `PACK_TARGET = 30720` (post-#121 amendment) now operates alongside T061's STRIP_TARGET as the second size-budget WARNING threshold on `make size` invocation. PR #119 added a CI gate (`Binary size budget gate` step in `.github/workflows/ci.yml`) which hard-fails (exit 1) the workflow when `linux_synth_packed` exceeds PACK_TARGET, so the v1.2.0 cap is machine-enforced — not just Makefile-asserted. The `make size` invocation's binary-sizes.txt artifact is now the canonical source for both T061's STRIP_TARGET check and the new PACK_TARGET check; the CI gate parses the same artifact directly. The current cap is ≤30 KB per `.specify/memory/constitution.md` Principle I amendment paragraph. (The "T061 ... (24 KB stripped target)" task line above is preserved as the spec-at-time-of-authoring claim per the integrity-preserving pattern; the underlying threshold referenced by Makefile has since been bumped to STRIP_TARGET = 51 200 per Constitution v1.1.0 / PR #116.)
