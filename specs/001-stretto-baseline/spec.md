...
- **2026-07-08** — Constitution v1.0.x → v1.1.0: Principle I Linux stripped binary budget bumped ≤24 KB → ≤50 KB per PR #116 (019-realign-strip-target). The Makefile `STRIP_TARGET` bumped 24576 → 51200 (~50 KB). **No new FR added** — v1.1.0 is a structural Principle I amendment to the existing budgeted size cap, not a new functional requirement; FR-052 (the existing "SHOULD strip toward 24 KB soft target; not CI-failing") is preserved AS-IS per the integrity-preserving pattern. The 24 KB FR-052 soft-target is now arguably superseded by the Constitution's ≤50 KB hard cap, but the FR set is append-only and the historical SHOULD soft-target is the spec-at-time-of-authoring claim; the new Constitution cap is the harder live rule (per PR #119 CI gate). The current cap is ≤50 KB per `.specify/memory/constitution.md` Principle I amendment paragraph. (PR #117's v1.1.0 spec-doc propagation did not refresh on disk for this 001 baseline spec; PR #124 / this PR closes the catchup by appending this entry to the Amendment-history footer without editing FR-052.)

- **2026-07-08** — Constitution v1.1.0 → v1.2.0: Principle I Linux UPX-packed binary budget added ≤30 KB per PR #121. No new FR added (no FR-052-equivalent for Linux UPX); v1.2.0 adds a structural Principle I budget enumeration explicitly where the prior implicit cap lived through Makefile `PACK_TARGET = 12288` (pre-#121) being half-reality. The 003 chain's cross-platform MIDI support added the proportional ~13 KB UPX-packed growth on top of the historical pre-#117 Linux stripped `~40 KB` measurement noted in FR-052 above. The current cap is ≤30 KB per `.specify/memory/constitution.md` Principle I amendment paragraph. (The ≤24 KB target in FR-052 above is preserved as the spec-at-time-of-authoring claim per the integrity-preserving pattern.)
