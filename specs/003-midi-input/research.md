...
**Decision** *(preflight-corrected 2026-07-06 — see Preflight Correction below)*: **`pthread_create()` + worker thread looping on blocking `snd_seq_event_input()`** + per-event parse into `midi_event_t`. The pthread blocks on `snd_seq_event_input(seq, &ev)` in ALSA's sequenced event context until an event arrives, parses the structured `snd_seq_event_t` payload (no manual status-byte nibble extraction — libasound gives `ev->type` for NoteOn / NoteOff / Controller / Port-exit classification and `ev->data.note.{note, velocity, channel}` / `ev->data.control.{param, value, channel}` directly), enqueues a `midi_event_t` to the shared ring buffer, and frees the event with `snd_seq_free_event()`.

**Preflight Correction (2026-07-06)**: the earlier draft mentioned `snd_seq_create_thread()` which is **not a function in stock libasound**; only `snd_seq_dispatch_set_callback()` exists for callback registration, and it requires an external polling loop on the audio thread (which would violate FR-033). The corrected std-libasound pattern is the pthread + blocking-input approach documented above. This adds **one pthread per process when `--midi` is set**, which is acceptable per Constitution V ('library-managed concurrency where possible; otherwise a single user-managed thread') and Constitution II (C99 — gcc + libpthread is already pulled in via `-lpulse`; no new library dependency). The pthread is started in `audio_midi_linux_init()` and joined in `audio_midi_linux_close()`.

**Rationale**: This matches the spec's Q2 answer (callback-only on both platforms, see `spec.md:14`) — callback-style dispatch where the audio thread is never blocked on platform-specific I/O. The pthread worker thread owns the libasound event-loop context; the audio thread only reads the shared ring buffer. The callback in the original question is no longer quite accurate — instead, the worker thread parses each event synchronously and calls `audio_midi_enqueue()` from pthread context, which is producer-context-safe (the SPSC ring buffer's `__atomic_store_n` with `__ATOMIC_RELEASE` is the producer-side fence, per D3). Audio thread remains the only consumer (per FR-033), reading head with `__atomic_load_n(__ATOMIC_ACQUIRE)` in `audio_midi_drain()`. ALSA libasound is dynamically linked via `-lasound` in the `synth` and `synth_cov` Makefile targets (mirrors the existing `-lpulse` dynamic-link pattern at `Makefile:104`, `Makefile:207`).
...
**Struct layout preservation (refinement 2026-07-06)**: re-use the existing `uint16_t _glide_pad;` field in `Voice` (currently unused padding before the union) to hold `trigger_key` and `trigger_channel` as two `uint8_t` halves. The struct grows by **zero bytes** (`_glide_pad` was already 2 bytes of tail padding), preserving `arena_alloc(N_VOICES * sizeof(Voice))` footprint and keeping the bitexact 16-s SHA-256 golden at `golden/regression_16s.sha256` byte-identical for the `--no-midi` path (FR-053). Generative-triggered voices leave these fields at zero (so non-MIDI voices can be distinguished from MIDI voices in code review).
