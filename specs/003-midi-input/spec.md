...
- Q2: Which threading model should the MIDI handler use? → A: **Callback-only on both platforms**. Linux uses `snd_seq_create_thread()` with a callback handler; Windows uses `midiInProc`. Both callbacks enqueue to the same shared ring buffer; the audio thread (existing) consumes the buffer at the start of each `render_chunk`. This keeps the producer/consumer pattern identical across platforms, avoids introducing a separate pthread on Linux (preserves the 24 KB binary budget), and means the audio thread never blocks on platform-specific I/O.
...
#### Cross-platform
- **FR-030**: Linux implementation MUST use the ALSA sequencer API (`snd_seq_*`) to enumerate + open MIDI input. Input MUST be read via an **ALSA-managed callback thread** spawned with `snd_seq_create_thread()`; the callback handler enqueues events to the shared ring buffer (per Clarifications 2026-07-06 Q2) and MUST NOT call into the audio thread directly.
