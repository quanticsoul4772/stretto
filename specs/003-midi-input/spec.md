...
- **FR-020**: A CC message on the active channel with controller C (0..127) and value V (0..127) MUST modulate a live parameter via a static mapping table:
  ...
  - **Unassigned CCs** (kept at `CC_TARGET_NONE` per Principle VII): CC#16 / CC#17 / CC#19 (General Purpose 1–4). Silently dropped with no `fprintf` and no callback overhead. (CC#123 was in this list until the 067 amendment routed it — see FR-023.)
...
- **FR-023** (amended 2026-07-11, 065 — gate semantics + sustain pedal):
  - A MIDI-triggered voice MUST hold at the sustain level after its decay completes, until its Note Off arrives (standard keyboard gate semantics — a held key rings). The generative scheduler's voices keep the original fire-and-forget auto-release; MIDI is never active in `--render`, so seeded renders are unaffected (FR-050/FR-053 byte-identity preserved).
  - CC#64 (Sustain Pedal) MUST use raw value semantics per MIDI 1.0 — value ≥ 64 = pedal down, < 64 = pedal up — **per channel**. While the pedal is down on a channel, Note Off (including Note On velocity 0 per FR-011) on that channel MUST NOT release the voice; it marks it held. Pedal-up MUST release every voice held on that channel. The pedal moves no synth parameters.
  - Voice-stealing MAY steal held voices under pool pressure (standard pedal behavior); a mid-session disconnect (FR-034) leaves held voices to be reclaimed by the generative scheduler's normal slot churn.
  - (amended 2026-07-11, 067) CC#123 (All Notes Off) MUST act as a Note Off for every sounding MIDI voice on the event's channel, per strict MIDI 1.0 semantics: with the damper pedal down on that channel, those Note Offs mean HOLD — the notes convert to held and survive until pedal-up. The data byte is deliberately ignored (MIDI defines it as 0; any value triggers — liberal acceptance). CC#123 moves no synth parameters. A full panic is CC#64 value 0 followed by CC#123.
