...
- [ ] T022 [P] [US1] Implement `audio_midi_linux.c` ALSA sequencer backend: `snd_seq_open()` + `snd_seq_create_thread(NULL, SND_SEQ_EVENT_INPUT, realtime_cb, private)` (priority `SND_SEQ_PRIORITY_NORMAL`) + `snd_seq_connect_from(seq, port, src_client, src_port)` for the subscribed device. The callback extracts the MIDI status byte to determine Note On / Off / CC and the channel; populates a `midi_event_t` and calls `audio_midi_enqueue()`. Initialize via `audio_midi_linux_init(int device_index)` and tear down via `audio_midi_linux_close()` (called from `audio_midi_close()` in T013 chain).
...
- [ ] T041 [P] Update `ARCHITECTURE.md` to describe MIDI input as part of the input surface (alongside the existing `keys.c` keyboard dispatcher); add a header-level pointer to `specs/003-midi-input/spec.md` for the capability spec.
