...
| **II. C99 Only** | ⚠️ **VIOLATION — amendment required** | Zig source enters the tree; `zig/density.zig` and `zig/chord_progression.zig` are already on `main` via PR #183. **"No build system beyond GNU Make" stays true** — hybrid keeps Make and gcc-as-linker. `nm -u` on the produced objects is empty, so the runtime-dependency clause (libc + libpulse / winmm) holds unchanged. Principle II is not tagged NON-NEGOTIABLE; a v1.3.0 amendment suffices procedurally. See Complexity Tracking row 1. |
...
| **V. Cleanly Modular** | ⚠️ **PARTIAL — amendment required** | The one-way dependency graph is untouched; no module gains or loses an edge. Two clauses break: "one `.c`/`.h` pair" (becomes `.zig`/`.h`), and **"no `extern` declarations across module boundaries"** — which is precisely what Zig `export fn` / `extern fn` is. The `.h` remains the documented seam but stops *binding* the Zig side, and nothing replaces compile-time signature checking. Invisible at leaves; unavoidable at `gen`. See Complexity Tracking row 2. |
| **VI. Test Discipline (NON-NEGOTIABLE)** | ⚠️ **AT RISK — Phase 1 gates this** | Every unit test survives unmodified — verified, not assumed: every file in `tests/unit/` includes only `test.h` and `../../<module>.h`, none `#include`s a `.c` or names a file-scope static. But `make coverage` and `make test-asan` both derive object lists from `COV_SRCS`, a `.c` filename list with only `.c` pattern rules, so the first port PR **fails to build** in two required checks. And ported modules leave `-Wall -Wextra -Werror`, which lives in `SAN_FLAGS` and nowhere else. No port PR may land until Phase 1 resolves this. See Complexity Tracking row 3. |
...
**Gate result**: ⛔ **BLOCKED on Principle II and V amendments (Phase 3) and the Principle VI resolution (Phase 1).** No port PR lands before both.
...
| **VI. coverage + sanitizer methodology** | gcov cannot instrument Zig, and `-fsanitize=address` cannot instrument a `zig build-obj` output. | *Keeping a C copy of each ported module solely for coverage* is mechanically easiest and rejected outright: the gate would measure code that is not in the shipped binary, and the two copies would drift silently. Building the Zig object at `-OReleaseSafe` in the `build_san` tree only (that tree never touches the release binary, so zero size risk) recovers overflow and bounds checking, which is strictly more than ASan gave an uninstrumented object. |
