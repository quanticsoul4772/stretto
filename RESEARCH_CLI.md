...
| F5 packaging minimum (man page, tagged releases + checksums) | **Shipped** — 048 (`stretto.1` + `make install` + release.yml with version/cleanliness/size gates; v1.3.0 released 2026-07-09) |
| *2026-07-10 follow-up: install friction (deep-research, 22 verified claims)* | **Shipped** — 058 (`install.sh` curl\|sh with sha256 verification + offline tests + release drift gate; legible no-server error naming pipewire-pulse; ubi one-liner verified/documented. Research verdicts: keep libpulse dynamic — pipewire-pulse is the 2026 default and libpulse its documented client path; no static binaries — Godot precedent is dlopen fallback; ALSA fallback deferred until headless demand) |
| P1 `NO_COLOR` | **Shipped** — 052 (SGR stripped at the single write site; functional escapes kept) |
| P3 brew/AUR | **Shipped** — 053/056 (repo public 2026-07-09; `Formula/stretto.rb` self-tap live with the v1.3.0 sha256 pinned; `packaging/aur/PKGBUILD` pinned and ready for the maintainer's AUR push. Brew formula is Linux-only: no macOS audio backend exists) |
